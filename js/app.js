...
async function importConfig() {
    showImportBox(async (file) => {
        try {
            ...
            if (config.name !== 'CGRunTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'CGRunTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'CGRunTV-Settings_' + times + '.json');
}
